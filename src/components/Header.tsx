...
export default function Header(): JSX.Element {
        ...
        function closePopUpOnClickOutside(e: MouseEvent): void {
            if (canShowNav && popUpMobileMenu && popUpMenuBtn) {
                ...
                const isClickOutsidePopup =
                    !popUpMobileMenu?.contains(clickedElement) &&
                    !popUpMenuBtn?.contains(clickedElement);
                if (isClickOutsidePopup && isMobileNavMenuOpen)
    ...
    return (
        <header className="fixed inset-[0 auto] z-10 flex justify-between items-center w-full px-5 md:px-20 mt-5 flex-none max-w-[1440px]">
            <Link
                to="/"
                className="w-fit h-[72px] bg-off-white rounded-full overflow-hidden">
                <img
                    className="object-top"
                    src={victoria}
                    width={72}
                    height={89.92}
                    alt="Oyelola victoria"
                />
            </Link>

            {canShowNav && (
                <nav
                    ref={popUpMobileMenuRef}
                    className="absolute right-5 md:right-20 top-[84%] lg:static bg-[#33333330] backdrop-blur-[2px] rounded-lg shadow-container border border-white/30">
                    <ul className="flex flex-col lg:flex-row gap-4 md:gap-8 p-4 md:px-8 md:py-5">
                        <li>
                            <Link
                                to=".#projects"
                                className="font-satoshi text-base text-off-white metallic-border hover:bg-cta-primary hover:bg-clip-text hover:text-transparent font-medium !leading-none tracking-[.01em]">
                                Projects
                            </Link>
                        </li>
                        <li>
            ...
            <div className="flex items-center gap-4 md:gap-8">
                <nav>
                    <ul className="flex gap-4 md:gap-8">
                        <li>
                            <a
                                className="block first:w-7 first:h-7"
                                href="https://linkedin.com/in/victoriaoyelola"
                                aria-label="Link to Oyelola Victoria's LinkedIn profile"
                                target="_blank">
                                {logos.linkedin}
                            </a>
                        </li>
                        <li>
                            <a
                                className="block first:w-7 first:h-7"
                                href="https://github.com/VriaA"
                                aria-label="Link to Oyelola Victoria's GitHub profile"
                                target="_blank">
                                {logos.github}
                            </a>
                        </li>
