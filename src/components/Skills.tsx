                    ...
                    <div className="flex justify-center flex-wrap max-w-[335px] md:max-w-[1019px]">
                        {skills.map((skill, i) => {
                            const RIGHT_BORDER =
                                (i + 1) % 5 !== 0 && "xl:border-r";
                            const BOTTOM_BORDER =
                                i < skills.length - 5 && "xl:border-b";
                            return (
                                <p
                                    key={`skill-${i}`}
                                    className={`skill flex justify-center items-center gap-2 w-16 h-16 md:w-[200px] md:h-24 text-base xl:text-lg font-satoshi font-medium !leading-none tracking-[.01em] bg-metallic-text text-transparent bg-clip-text metallic-border ${RIGHT_BORDER} ${BOTTOM_BORDER}`}>
                                    {skill.logo}
