        ...
        <section className="relative flex flex-col items-center z-[2] text-white mt-20 mb-10 md:mb-20">
            ...
            <div className="flex flex-col gap-5 md:gap-10 mt-10">
                ...
                {projects.map((project, i)=> {
                    ...
                    return (
                    <div className="flex flex-col items-center w-full bg-metallic-gradient p-[1px] rounded-2xl overflow-hidden">
                        <div key={`project-${i + 1}`} className="flex flex-col lg:flex-row gap-10 items-center md:items-end justify-between h-fit bg-black rounded-2xl px-5 md:px-10 py-10">
                            <img className="order-0 lg:order-2 w-[300px] md:w-[540px] h-fit" src={imageSrc} alt={`Oyelola Victoria's ${name} Project`} />

                            <div className="order-1 lg:order-0 flex flex-col gap-10 lg:gap-20 justify-between h-fit w-[50%]">
                                {isCompleted ?
                                    <div className="flex gap-4">
                                            <a href={links.website} target="_blank">
                                                <svg className="w-fit h-6 lg:h-8" viewBox="0 0 24 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <a href={links.github} target="_blank">
                                                <svg  className="w-fit h-6 lg:h-8" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div>
                                        ...
                                        <p className="mt-3 font-satoshi font-medium text-sm lg:text-lg leading-[150%] tracking-[.01em]">{description}</p>
                                        
                                        <div className="flex flex-wrap gap-1 lg:gap-2 w-full mt-5">
                                            {technologiesUsed.map((tech)=> {
                                                const { name, logo } = tech
                                                return (
                                            <p className="project-stack flex items-center gap-1 w-fit px-2 py-1 lg:px-4 lg:py-2 h-8 lg:h-9 font-satoshi font-medium text-sm lg:text-base tracking-[.01em] bg-metallic-text text-transparent bg-clip-text !leading-none rounded-2xl border bg-dark-gray/20 border-white/30 shadow-container">
                                                {logo}
                                                {name}
                                            </p>
                                                )
