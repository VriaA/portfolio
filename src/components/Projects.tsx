        ...
        <section id="projects" className="relative flex flex-col items-center py-10 md:py-20 z-[2] text-white">
            ...
            <div className="flex flex-col gap-5 lg:gap-10 mt-5 md:mt-10 w-full lg:flex-row lg:flex-wrap xl:flex-row ">
                {projects.map((project, i) => {
                    const { name, description, technologiesUsed, imageSrc, links, isCompleted, isOpenSource, isFreelance } = project
                    return (
                        <div key={`project-${i + 1}`} className="flex flex-col items-center lg:w-[calc(50%-20px)] xl:w-full bg-metallic-gradient p-[1px] rounded-2xl overflow-hidden">
                            <div className="flex flex-col xl:flex-row gap-10 items-center xl:items-end xl:justify-between w-full h-full bg-black rounded-2xl px-5 md:px-10 py-10">
                                <img className="order-0 xl:order-2 w-[300px] lg:w-[540px] h-fit" src={imageSrc} alt={`Oyelola Victoria's ${name} Project`} />
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-1 lg:gap-2 w-full mt-5">
                                            {technologiesUsed.map((tech) => {
                                                ...
                                                return (
                                                    <p key={`${name}-${techName}`} className="project-stack flex items-center gap-1 w-fit px-2 py-1 xl:px-4 xl:py-2 h-8 xl:h-9 font-satoshi font-medium text-base lg:text-base tracking-[.01em] bg-metallic-text text-transparent bg-clip-text !leading-none rounded-2xl border bg-dark-gray/20 border-white/30 shadow-container">
                                                        {logo}
                                                        {techName}
                                                    </p>
                                                )
            ...
            {
                !isProjectsPage &&
                <Link to="/projects" className="group flex items-center gap-2 mt-10 border-b border-b-white pb-1 font-satoshi font-medium text-lg lg:text-xl text-transparent bg-clip-text bg-cta-primary tracking-[0.01em]"><ShinyText text="See all projects" speed={1} />
                    <svg className="w-5 animate-right group-hover:animate-none" fill="#AEB2FA" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" aria-label="Right arrow icon">
